...
limite_distrital_centroide <-
  st_point_on_surface(limite_distrital)
...
Unión de distritos {data-orientation=rows}
===================================== 
Column {data-width=450}
-----------------------------------------------------------------------
...
# Mapa Leaflet con capas de distitos, red vial y actividades comerciales en Santa Ana
    ...
    leaflet() %>%
      addProviderTiles(providers$CartoDB.DarkMatter, group = "Mapa base") %>%
      addProviderTiles(providers$Esri.WorldImagery, group = "Imagen Satelital") %>%
      addPolygons(
        data = limite_distrital,
        color = "#8B3E2F",
      ...
      addPolygons(
        data = distritos_sa_unifi,
        color = "#8B3E2F",
        stroke = TRUE,
        weight = 3.0,
        group = "Distritos unidos"
      ) %>%
      addControlGPS() %>%
      addSearchOSM() %>%
      addResetMapButton() %>%
      ...
      addLayersControl(
        baseGroups = c("Mapa base", "Imagen Satelital"),
        overlayGroups = c("Limite distrital", "Distritos unidos"),
        options = layersControlOptions(collapsed = FALSE)
      )    
...
```{r Buffer en ruta 121 cantón Santa Ana}
